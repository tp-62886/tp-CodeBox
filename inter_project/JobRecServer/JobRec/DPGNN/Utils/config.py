...
class PJFConfig(Config):
    ...
    def _load_internal_config_dict(self, model, model_class, dataset):
        ...
        overall_init_file = os.path.join(current_path, '../yamlFile/overall.yaml')
        model_init_file = os.path.join(current_path, '../yamlFile/' + model + '.yaml')
        dataset_init_file = os.path.join(current_path, '../yamlFile/' + dataset + '.yaml')
