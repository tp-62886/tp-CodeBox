...
from JobRec.DPGNN.Dataset.dataloader import *

def load_trained_model(config, model_file_path, dataset):
    # mapid =dataset.original_to_mapped['job_id']['91650000MA7781KU5D']
    train_data, valid_data, test_data = data_preparation(config, dataset)

    # print(len(train_data.dataset))
    # print(len(valid_data[0].dataset))
    # print(len(valid_data[1].dataset))
    # print(len(test_data[0].dataset))
    # print(len(test_data[1].dataset))
...
def initialize_new_embeddings(model, num_new_users, new_num_items):
    ...
    model.user_embedding_a = new_user_embedding_a.to(config['device'])
    model.item_embedding_a = new_item_embedding_a.to(config['device'])
    model.user_embedding_p = new_user_embedding_p.to(config['device'])
    model.item_embedding_p = new_item_embedding_p.to(config['device'])
...
'''更新BERT嵌入数据'''
def update_bert_embeddings(model, new_dataset, extracted_user_values, extracted_job_values):
    # 获取现有的 BERT 嵌入
    ...
    new_bert_item = new_bert_item[extracted_job_values]
    new_bert_user = new_bert_user[extracted_user_values]

    # 去除第一行的空值
    new_bert_item = new_bert_item[1:] if new_bert_item[0].sum() == 0 else new_bert_item
    new_bert_user = new_bert_user[1:] if new_bert_user[0].sum() == 0 else new_bert_user
    print("new_bert_item.shape:", new_bert_item.shape)
    print("new_bert_item.shape:", new_bert_user.shape)
    ...
    # 更新模型的 BERT 嵌入
    model.bert_user = updated_bert_user.to(config['device'])
    model.bert_item = updated_bert_item.to(config['device'])

def filter_mapped_ids_by_category(original_to_mapped, original_to_mapped2, category):
    filtered_category = {}
    extracted_values = []

    if category in original_to_mapped and category in original_to_mapped2:
        for key, value in original_to_mapped2[category].items():
            if key not in original_to_mapped[category]:
                filtered_category[key] = value
                extracted_values.append(value)

    return filtered_category, extracted_values
...
num_new_items = 0  # 假设你有新的项目数量

print("=====2、加载已训练好的模型=====")
trained_model_path = r'D:\1-work-code\JobRecProject_v2\JobRec\DPGNN\model\saved\DPGNN-Sep-10-2024_23-29-08.pth'
dataset = PJFDataset(config)
model, valid_data = load_trained_model(config, trained_model_path,dataset)

print("=====3、处理新增加的训练数据=====")
new_config = PJFConfig(model='DPGNN', dataset='xdu_incre', config_file_list=None, config_dict=None)
...
print("==4.2 更新BERT嵌入==")
# 筛选出需要保留的user_id对应的mapID
filtered_user_ids, extracted_user_values = filter_mapped_ids_by_category(
    dataset.original_to_mapped, new_dataset.original_to_mapped, 'user_id'
)
# 筛选出需要保留的job_id对应的mapID
filtered_job_ids, extracted_job_values = filter_mapped_ids_by_category(
    dataset.original_to_mapped, new_dataset.original_to_mapped, 'job_id'
)
num_new_users = len(extracted_user_values)
num_new_items = len(extracted_job_values)
# 更新 BERT 嵌入
update_bert_embeddings(model, new_dataset, extracted_user_values, extracted_job_values)
