...
class DPGNN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        from JobRec.DPGNN.model.layer import GCNConv
    ...
    def get_ego_embeddings(self, interaction, geek_field, job_field):
        ...
        # 将句子通过嵌入层转换为向量表示
        geek_vecs, job_vecs = self.emb(geek_sents), self.emb(
            job_sents)  # 256,20,30,128 batch_size,max_sent_num,max_sent_len,embedding_size
        ...
        # 获取用户和项目的数量
        n_users = user_embeddings_a.shape[0]
        n_items = item_embeddings_p.shape[0]

        # 通过重复数据调整 gr 和 gj 的大小以匹配用户和项目的数量
        gr = gr.repeat((n_users // gr.size(0)) + 1, 1)[:n_users]  # 调整 gr 大小以匹配用户数量
        gj = gj.repeat((n_items // gj.size(0)) + 1, 1)[:n_items]  # 调整 gj 大小以匹配项目数量
    ...
    def info_nce_loss(self, index, interaction, geek_field, job_field, is_user):
        all_embeddings = self.get_ego_embeddings(interaction, geek_field, job_field)
    ...
    def calculate_loss(self, interaction):
        ...
        user_e_a, item_e_p, user_e_p, item_e_a = self.forward(interaction, self.USER_SENTS, self.ITEM_SENTS)
        ...
        logits_user, labels = self.info_nce_loss(user, interaction, self.USER_SENTS, self.ITEM_SENTS, is_user=True)
        loss += self.mul_weight * self.mutual_loss(logits_user, labels)

        logits_job, labels = self.info_nce_loss(item, interaction, self.USER_SENTS, self.ITEM_SENTS, is_user=False)
