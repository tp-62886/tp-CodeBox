...
class DPGNN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        '''
        2.1、加载compute_globalhidden相关参数
        '''
    ...
    def compute_globalhidden(self, hidden):
        # 创建掩码，根据实际数据生成
        mask = torch.ones(hidden.shape[0], hidden.shape[1], dtype=torch.float32).to(self.device)  # 这里假设全为1
    ...
    def forward(self):
        ...
        # 结合 compute_globalhidden 方法处理用户嵌入和项目嵌入
        user_e_a = self.compute_globalhidden(user_e_a)
        item_e_p = self.compute_globalhidden(item_e_p)
        user_e_p = self.compute_globalhidden(user_e_p)
        item_e_a = self.compute_globalhidden(item_e_a)
