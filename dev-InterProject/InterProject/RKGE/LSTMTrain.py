...
class LSTMTrain(object):
    ...
    def dump_post_embedding(self):
        ...
        for node in node_list:
            node_id = torch.LongTensor([int(self.all_variables[node])]).to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.iteration):
            ...
            for pair in self.paths_between_pairs:
                ...
                paths_between_one_pair_id = Variable(torch.LongTensor(paths_between_one_pair_id)).to(self.device)
                #################################################################################################
                out = self.model(paths_between_one_pair_id)  # 执行LSTMTagger的前向传播
                out = out.squeeze()

                if pair in self.positive_label:
                    label = Variable(torch.Tensor([1])).squeeze().to(self.device)
                else:
                    label = Variable(torch.Tensor([0])).squeeze().to(self.device)  # 7_18
                    # label = Variable(torch.Tensor([-1])).squeeze().to(self.device)
...
#
# 				if pair in self.positive_label:
# 					label = Variable(torch.Tensor([1])).squeeze()
# 				else:
# 					label = Variable(torch.Tensor([0])).squeeze()
#
...
# 		return self.dump_post_embedding()
	...
	#
	# 			if pair in self.positive_label:
	# 				label = Variable(torch.Tensor([1])).unsqueeze(0)
	# 			else:
	# 				label = Variable(torch.Tensor([0])).unsqueeze(0)
	#
	# 			loss = criterion(out.cpu(), label)
