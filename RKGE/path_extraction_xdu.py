...
def dump_paths(Graph, rating_pair, maxLen, sample_size, fw_file):
    ...
    for pair in rating_pair:
        ...
        if Graph.has_node(user_node) and Graph.has_node(work_node):
            num = num + 1
            if nx.shortest_path_length(Graph, source=user_node, target=work_node) > maxLen + 1:
...
def main(training_file, negative_file, auxiliary_file, positive_path, negative_path, path_length, sample_size):
    ...
    with open(negative_file, 'r', encoding='ascii') as fr_negative:
    ...
    print(f"路径挖掘完毕，耗时{spend_time}分钟")
...
#
#     parser.add_argument('--training', type=str, dest='training_file', default='data/xdu/training.txt')
#     parser.add_argument('--negtive', type=str, dest='negative_file', default='data/xdu/negative.txt')
#     parser.add_argument('--auxiliary', type=str, dest='auxiliary_file', default='data/xdu/auxiliary-mapping.txt')
